...
def training(
    ...
    for iteration in range(first_iter, opt_params["iterations"] + 1):    
        ...
        if opt_params["metric_xyz"]:
            ...
            losses['xyz'] = F.relu(gaussians._xyz[visibility_filter.all(dim=-1)].norm(dim=1) - opt_params["threshold_xyz"]).mean() * opt_params["lambda_xyz"]

        if opt_params["lambda_scale"] != 0:
            if opt_params["metric_scale"]:
                losses['scale'] = F.relu(gaussians.get_scaling[visibility_filter.all(dim=-1)] - opt_params["threshold_scale"]).norm(dim=1).mean() * opt_params["lambda_scale"]
            else:
                losses['scale'] = F.relu(torch.exp(gaussians._scaling[visibility_filter.all(dim=-1)]) - opt_params["threshold_scale"]).norm(dim=1).mean() * opt_params["lambda_scale"]
